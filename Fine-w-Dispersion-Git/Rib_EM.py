...
domain_y = domain_x
# Waveguide widths.
rib_w_ls = [1.25, 1.50, 1.75, 2.00, 2.60]
rib_w_ls = [x * 1e3 for x in rib_w_ls]
...
'''
0.8 --> 2.5 um widths at 20 nm steps

'''
p = 1

while rib_w <= 2500:
    if rib_w >= 1500 & rib_w <= 2000:
        p = 0.9
    elif rib_w > 2000:
        p = 0.8

    
    output_folder = 'Rib_{}'.format(rib_w)
    os.makedirs(output_folder, exist_ok=True)
    
    AC_mode_data_prefix = os.path.join(f'{output_folder}', 'Rib_Field_data', 'AC_Mode_Data.csv')
    os.makedirs(os.path.dirname(AC_mode_data_prefix), exist_ok=True)
    
    EM_mode_data_prefix = os.path.join(f'{output_folder}', 'Rib_Field_data', 'EM_Mode_Data.csv')
    ...
    prefix, refine_fac = starter.read_args(rib_w, sys.argv)
    nbapp = numbat.NumBATApp(prefix, outdir=output_folder)
    print(f"This is for rib width: {rib_w}\n")
    ...
    sim_EM_pump.plot_modes(xlim_min=p*((rib_w - 175) / (2 * rib_w)), xlim_max=p*((rib_w - 175) / (2 * rib_w)), 
                           ylim_min=0.6, ylim_max=0.3, 
                           mode_indices=[EM_mode_index_pump], field_type='EM_E',
                           num_ticks=3, ticks=True, colorbar=True, 
                           quiver_points=40, n_points=1000, suffix = f'_{rib_w}')
    ...
    sim_AC.plot_modes(num_ticks=3, 
                      xlim_min=p*((rib_w - 175) / (2 * rib_w)), xlim_max=p*((rib_w - 175) / (2 * rib_w)), 
                      ylim_min=0.88, ylim_max=-0.07,  
                      suffix = f'_{rib_w}')
    ...
    gain_box.plot_spectra(freq_min=freq_min, freq_max=freq_max, logy=False, suffix = f'_{rib_w}')
